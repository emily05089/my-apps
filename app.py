...
<head>
    ...
    <style>
        ...
        .btn-del:active { background: #a71d2a; }
...
<body>
    <div class="container">
        ...
        <div id="fileListContainer" class="file-list">
            <div style="color:#999; text-align:center; padding:10px;">暂无文件，请点击“添加图片/PDF”</div>
        </div>
        <div id="loading" class="loading">⏳ 正在拼命识别中，请稍候...</div>
    ...
    <script>
        ...
        function handleFiles(files) {
            ...
            if(selectedFiles.length === 0 && files.length === 0){
                listContainer.innerHTML = '<div style="color:#999; text-align:center; padding:10px;">暂无文件</div>';
                return;
            }
            // 将新上传的文件追加到数组中
            ...
            try:
                if file_name.endswith('.pdf'):
                    ...
                    for page_num in range(len(pdf_doc)):
                        ...
                        full_result.append(parse_format(parse_image_to_text(img)))
                elif file_name.endswith(('.jpg', '.png', '.jpeg')):
                    img = Image.open(file.stream)
                    full_result.append(parse_format(parse_image_to_text(img)))
            except Exception as e:
                full_result.append(f"【文件 {file.filename} 识别出错】")
        result_text = "\n".join(full_result)
